    ...
    if (IsEmpty == FALSE) {
          ...
          ((FaultRecord.Fault[0] & 0xFF) == 0x12) || ((FaultRecord.Fault[0] & 0xFF) == 0x13))
      {
        ...
        if ((SteSlot != NULL) && (SteSlot->Valid != 0) && (FaultRoot != NULL)) {
          ...
          DEBUG ((DEBUG_ERROR, "%a: StreamId=0x%x has no page-table root (STE still in abort mode)\n", __func__, FaultStreamId));
        }
      }
    }
  } while (IsEmpty == FALSE);
            ...
            if (SmmuBase != NULL) {
              ...
              if (SmmuNode->Node.Type == EFI_ACPI_IORT_TYPE_SMMUv3) {
                *SmmuBase = SmmuNode->Base;
                ...
                DEBUG ((DEBUG_WARN, "%a: OutputReference does not point to an SMMUv3 node (type=%u)\n", __func__, SmmuNode->Node.Type));
                *SmmuBase = 0;
              }
            }
  ...
  )
{
  ...
  for (NodeIdx = 0; NodeIdx < Iort->NumNodes; NodeIdx++) {
    if (Node->Type == EFI_ACPI_IORT_TYPE_NAMED_COMP) {
      ...
      if ((NameMax > 0) && (AsciiStrnCmp (NodeName, WantedName, NameMax) == 0) &&
  ...
  )
{
  ...
  if ((IortTable == NULL) || (DeviceHandle == NULL) || (StreamIdList == NULL)) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "%a: No PciIo on handle\n", __func__));
    return EFI_UNSUPPORTED;
  }
  ...
  if ((UINT16)Seg != 0x00FF) {
    ...
    DEBUG ((
      DEBUG_VERBOSE,
      "%a: PCIe S%04x B%02x D%02x F%02x --> StreamID=0x%x SmmuBase=0x%llx\n",
      __func__,
      (UINT32)Seg,
      (UINT32)Bus,
      (UINT32)Dev,
  ...
  //
  DEBUG ((DEBUG_VERBOSE, "%a: NonDiscoverable device (Seg=0xFF Bus=%u Dev=%u)\n", __func__, (UINT32)Bus, (UINT32)Dev));
