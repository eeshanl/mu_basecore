  ...
  )
{
  ...
  if (Ste->Valid != 0) {
    return SmmuV3DecodeSte (Ste, OutRoot, OutVmid);
  }
  ...
  )
{
  ...
  for (int i = 0; i < mIoMmu->SmmuCount; i++) {
    if (mIoMmu->SmmuInfo[i].Enabled) {
      SmmuV3LogErrors (&mIoMmu->SmmuInfo[i]);
    }
  }
  ...
  )
{
  ...
  // IOMMU not enabled, noop.
  if (IommuBase == 0) {
    DEBUG ((DEBUG_VERBOSE, "%a: IommuBase is 0, IoMmu mapping is not enabled\n", __func__));
    return EFI_SUCCESS;
  }
